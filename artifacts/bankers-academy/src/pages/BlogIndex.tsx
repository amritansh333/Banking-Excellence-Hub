...
function BlogImage({ blog, compact = false }: { blog: BlogPost; compact?: boolean }) {
  ...
  return (
    <div className="hero-gradient flex h-full w-full items-center justify-center px-5 text-center text-white">
      <div>
        <p className={`font-serif font-bold ${compact ? "text-base" : "text-2xl"}`}>The Bankers Academy</p>
...
function ArticleCard({ blog, featured = false }: { blog: BlogPost; featured?: boolean }) {
  return (
    <Link
      href={`/blog/${blog.slug}`}
    ...
    >
      <div className={`overflow-hidden bg-[#0B1F4D]/10 ${featured ? "aspect-[16/10] lg:aspect-auto lg:w-[42%]" : "aspect-[16/10]"}`}>
        <BlogImage blog={blog} />
      </div>
      <div className="flex flex-1 flex-col p-5 md:p-6">
        ...
        <h2 className={`font-serif font-bold leading-tight text-[#0B1F4D] transition-colors group-hover:text-[#C89B3C] ${featured ? "text-2xl md:text-3xl" : "text-xl"}`}>
          {blog.title}
        </h2>
        {blog.excerpt && <p className="mt-3 line-clamp-3 text-sm leading-7 text-gray-600">{blog.excerpt}</p>}
        ...
        <div className="mt-auto flex flex-col gap-4 pt-5 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex flex-wrap gap-2">
            {blog.tags.slice(0, 3).map((tag) => (
              <span key={tag.id} className="inline-flex items-center gap-1 rounded bg-[#F3F5F8] px-2 py-1 text-xs text-gray-600">
                <Tag size={11} />
                {tag.name}
              </span>
            ))}
...
function SidebarLink({ blog, index }: { blog: BlogPost; index: number }) {
  return (
    <Link href={`/blog/${blog.slug}`} className="group grid grid-cols-[36px_1fr] gap-3">
...
export default function BlogIndex() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchPublicBlogCategories(), fetchPublicBlogTags(), fetchPopularBlogs(5)])
    ...
    <div className="overflow-hidden bg-[#F8FAFC]">
      <section className="hero-gradient relative overflow-hidden py-14 text-white md:py-20">
        <div className="container relative z-10 mx-auto px-4 text-center md:px-6">
          <motion.div initial="hidden" animate="visible" variants={stagger} className="mx-auto max-w-3xl space-y-5">
            <motion.span variants={fadeUp} className="inline-block rounded-full border border-[#C89B3C]/40 bg-[#C89B3C]/15 px-4 py-2 text-sm font-semibold text-[#E8C879]">
              Insights & Resources
            </motion.span>
            <motion.h1 variants={fadeUp} className="font-serif text-4xl font-bold leading-tight md:text-6xl">
              Blog & Articles
            </motion.h1>
            <motion.p variants={fadeUp} className="mx-auto max-w-2xl text-base leading-8 text-blue-100 md:text-lg">
              Industry insights, career guidance, and private banking knowledge from our expert faculty.
            </motion.p>
          </motion.div>
        </div>
        <div className="absolute bottom-0 left-0 right-0">
          <svg viewBox="0 0 1440 54" fill="none" aria-hidden="true">
            <path d="M0 54H1440V0C1190 34 960 52 720 52C480 52 250 34 0 0V54Z" fill="#F8FAFC" />
          </svg>
        </div>
      </section>
          ...
          <div className="mb-8 flex flex-wrap gap-2">
            <button
              ...
              className={`rounded-full border px-4 py-2 text-sm font-semibold transition-all ${
                activeCategory === "" ? "border-[#0B1F4D] bg-[#0B1F4D] text-white" : "border-gray-200 bg-white text-gray-600 hover:border-[#C89B3C]"
              }`}
            >
              All
            </button>
          ...
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
            <main className="min-w-0">
              <motion.div initial="hidden" animate={gridInView ? "visible" : "hidden"} variants={stagger} className="space-y-6">
                {loading && <p className="rounded-lg border border-gray-200 bg-white p-6 text-sm text-gray-600">Loading articles...</p>}
                {error && <p className="rounded-lg border border-red-200 bg-red-50 p-6 text-sm text-destructive">{error}</p>}
            ...
            <aside className="space-y-6 lg:sticky lg:top-32 lg:self-start">
              <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                <h2 className="mb-4 font-serif text-lg font-bold text-[#0B1F4D]">Search Articles</h2>
              ...
              {popular.length > 0 && (
                <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                  <h2 className="mb-4 font-serif text-lg font-bold text-[#0B1F4D]">Popular Articles</h2>
              ...
              {categories.length > 0 && (
                <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                  <h2 className="mb-4 font-serif text-lg font-bold text-[#0B1F4D]">Categories</h2>
              ...
              {tags.length > 0 && (
                <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                  <h2 className="mb-4 font-serif text-lg font-bold text-[#0B1F4D]">Tags</h2>
                  <div className="flex flex-wrap gap-2">
                    {tags.map((tag) => (
                      <button
                        key={tag.id}
                        type="button"
                        onClick={() => setActiveTag(activeTag === (tag.slug ?? tag.name) ? "" : tag.slug ?? tag.name)}
              ...
              <div className="hero-gradient rounded-lg p-6 text-white shadow-sm">
                ...
                <p className="mt-2 text-sm leading-6 text-blue-100">Join our focused program and launch your private banking career.</p>
                <Link
                  href="/admission"
