...
router.get("/admin/permissions", requireAdminAuth, async (_req, res): Promise<void> => {
  res.json(PERMISSION_CATALOGUE);
});

router.get("/admin/personas", requirePermission(PERMISSIONS.USERS_VIEW), async (_req, res): Promise<void> => {
  const personas = await db.select().from(personasTable);
  res.json(personas);
});

router.get("/admin/audit-log", requirePermission(PERMISSIONS.AUDIT_LOG_VIEW), async (req, res): Promise<void> => {
